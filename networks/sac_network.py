...
class SoftActorCritic(torch.nn.Module):
    def __init__(self, env: Environment, hidden_size: int):
        ...
        self.q_map = torch.nn.Linear(hidden_size, env.action_space.n)
    ...
    def forward(
        ...
        q = self.q_map(features)
        return [policy, value, q]


class SoftActorCriticNetwork(SoftActorCritic):
    def __init__(self, env: Environment, hidden_size: int):
...
class ConvActorCriticNetwork(SoftActorCritic):
    def __init__(
        self,
        env: Environment,
        hidden_size: int,
